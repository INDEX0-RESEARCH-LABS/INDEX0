...
from index0_agent.core import logger
from index0_agent.core.config.agent_config import AgentConfig
from index0_agent.core.config.app_config import AppConfig
from index0_agent.core.config.config_utils import (
    OH_DEFAULT_AGENT,
    OH_MAX_ITERATIONS,
)
from index0_agent.core.config.llm_config import LLMConfig
from index0_agent.core.config.sandbox_config import SandboxConfig
from index0_agent.core.config.security_config import SecurityConfig
...
def load_from_env(cfg: AppConfig, env_or_toml_dict: dict | MutableMapping[str, str]):
    ...
    def set_attr_from_env(sub_config: Any, prefix=''):
        ...
        annotations = getattr(sub_config, '__annotations__', None) or getattr(type(sub_config), '__annotations__', {})
...
def load_from_toml(cfg: AppConfig, toml_file: str = 'config.toml'):
    ...
    try:
        ...
        for key in keys_to_migrate:
            ...
            sandbox_annotations = getattr(sandbox_config, '__annotations__', None) or getattr(type(sandbox_config), '__annotations__', {})
            if new_key in sandbox_annotations:
