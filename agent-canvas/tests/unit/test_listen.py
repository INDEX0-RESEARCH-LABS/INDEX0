...
import index0_agent.server.session

# Patch necessary components before importing from listen
with (
    patch('index0_agent.server.session.SessionManager', MockSessionManager),
    patch('fastapi.staticfiles.StaticFiles', MockStaticFiles),
):
    from index0_agent.server.file_config import (
...
def test_load_file_upload_config_invalid_max_size():
    ...
    with patch('index0_agent.server.shared.config', config):
...
def test_is_extension_allowed():
    with (
        patch('index0_agent.server.file_config.RESTRICT_FILE_TYPES', True),
        patch('index0_agent.server.file_config.ALLOWED_EXTENSIONS', ['.txt', '.pdf']),
    ):
...
def test_is_extension_allowed_no_restrictions():
    with patch('index0_agent.server.file_config.RESTRICT_FILE_TYPES', False):
        assert is_extension_allowed('file.txt')
        assert is_extension_allowed('file.pdf')
...
def test_is_extension_allowed_wildcard():
    with (
        patch('index0_agent.server.file_config.RESTRICT_FILE_TYPES', True),
        patch('index0_agent.server.file_config.ALLOWED_EXTENSIONS', ['.*']),
    ):
        assert is_extension_allowed('file.txt')
        assert is_extension_allowed('file.pdf')
        assert is_extension_allowed('file.doc')
