...
from index0_agent.memory.memory import LongTermMemory
...
def mock_event_stream(mock_file_store: FileStore) -> EventStream:
    with patch('index0_agent.events.stream.EventStream') as MockEventStream:
...
def test_load_events_into_index_with_invalid_json(
    ...
    with patch(
        'index0_agent.memory.memory.event_to_memory',
...
def test_search_returns_correct_results(long_term_memory: LongTermMemory):
    ...
    with patch(
        'index0_agent.memory.memory.VectorIndexRetriever', return_value=mock_retriever
    ):
...
def test_load_events_batch_insert(
    ...
    with patch('index0_agent.utils.embeddings.insert_batch_docs') as mock_run_docs:
        # convert events to documents
        documents = long_term_memory._events_to_docs()
