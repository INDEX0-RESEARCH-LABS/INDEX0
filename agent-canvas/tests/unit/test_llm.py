...
from index0_agent.core.exceptions import OperationCancelled
from index0_agent.core.message import Message, TextContent
from index0_agent.llm.llm import LLM
from index0_agent.llm.metrics import Metrics
...
def mock_logger(monkeypatch):
    ...
    monkeypatch.setattr('index0_agent.llm.debug_mixin.llm_prompt_logger', mock_logger)
    monkeypatch.setattr('index0_agent.llm.debug_mixin.llm_response_logger', mock_logger)
    monkeypatch.setattr('index0_agent.llm.llm.logger', mock_logger)
    return mock_logger
...
@patch('index0_agent.llm.llm.litellm.get_model_info')
...
@patch('index0_agent.llm.llm.litellm_completion')
@patch('time.time')
...
def test_llm_cloudflare_blockage(mock_litellm_completion, default_config):
    ...
    from index0_agent.core.exceptions import CloudFlareBlockageError
...
@patch('index0_agent.llm.llm.litellm.token_counter')
...
@patch('index0_agent.llm.llm.litellm.token_counter')
@patch('index0_agent.llm.llm.create_pretrained_tokenizer')
