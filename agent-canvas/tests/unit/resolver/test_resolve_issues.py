...
from index0_agent.events.action import CmdRunAction
from index0_agent.events.observation import CmdOutputObservation, NullObservation
from index0_agent.llm.llm import LLM
from index0_agent.resolver.github_issue import GithubIssue, ReviewThread
from index0_agent.resolver.issue_definitions import IssueHandler, PRHandler
from index0_agent.resolver.resolve_issue import (
    complete_runtime,
    initialize_runtime,
    process_issue,
)
from index0_agent.resolver.resolver_output import ResolverOutput
...
async def test_resolve_issue_no_issues_found():
    from index0_agent.resolver.resolve_issue import resolve_issue
    ...
    with patch(
        'index0_agent.resolver.resolve_issue.issue_handler_factory',
...
async def test_process_issue(mock_output_dir, mock_prompt_template):
    ...
    for test_case in test_cases:
        ...
        with (
            patch(
                'index0_agent.resolver.resolve_issue.create_runtime', mock_create_runtime
            ),
            patch(
                'index0_agent.resolver.resolve_issue.initialize_runtime',
                mock_initialize_runtime,
            ),
            patch(
                'index0_agent.resolver.resolve_issue.run_controller', mock_run_controller
            ),
            patch(
                'index0_agent.resolver.resolve_issue.complete_runtime',
                mock_complete_runtime,
            ),
            patch('index0_agent.resolver.resolve_issue.logger'),
        ):
            # Call the function
...
def test_file_instruction():
    ...
    # load prompt from index0_agent/resolver/prompts/resolve/basic.jinja
...
def test_file_instruction_with_repo_instruction():
    ...
    with open('openhands/resolver/prompts/resolve/basic.jinja', 'r') as f:
        prompt = f.read()
    # load repo instruction from index0_agent/resolver/prompts/repo_instructions/all-hands-ai___openhands-resolver.txt
    with open(
