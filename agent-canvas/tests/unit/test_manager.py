...
from index0_agent.server.session.manager import SessionManager
...
async def test_session_not_running_in_cluster():
    ...
    id = uuid4()
    with (
        patch('index0_agent.server.session.manager._REDIS_POLL_TIMEOUT', 0.01),
        patch('index0_agent.server.session.manager.uuid4', MagicMock(return_value=id)),
    ):
...
async def test_session_is_running_in_cluster():
    ...
    with (
        patch('index0_agent.server.session.manager._REDIS_POLL_TIMEOUT', 0.1),
        patch('index0_agent.server.session.manager.uuid4', MagicMock(return_value=id)),
    ):
        async with SessionManager(
            sio, AppConfig(), InMemoryFileStore()
...
async def test_init_new_local_session():
    ...
    with (
        patch('index0_agent.server.session.manager.Session', mock_session),
        patch('index0_agent.server.session.manager._REDIS_POLL_TIMEOUT', 0.1),
        patch(
            'index0_agent.server.session.manager.SessionManager._redis_subscribe',
            AsyncMock(),
        ),
        patch(
            'index0_agent.server.session.manager.SessionManager.is_agent_loop_running_in_cluster',
